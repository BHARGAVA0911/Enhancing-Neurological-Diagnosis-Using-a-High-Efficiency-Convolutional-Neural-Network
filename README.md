# Enhancing-Neurological-Diagnosis-Using-a-High-Efficiency-Convolutional-Neural-Network

In this work, we propose a dedicated CNN model for automating the initial screening procedure with a motive of improving the diagnostic efficiency and accuracy while also assisting physicians and radiologists in reaching quicker and more accurate choices. 

The model was extensively tested on an independent set of 300 distinct head CT scans sourced from a hospital. The dataset included 7,023 images from various neurological conditions, such as
1. Normal (658)
2. Stroke (950)
3. Glioma (300)
4. Meningioma (306)
5. Pituitary (300)
6. Notumour (405)
7. Haemorrhage (200) cases.

On the test data, the proposed CNN model performs remarkably well, with a low loss of 0.255 and an accuracy of 94.3%.
The model's near- perfect accuracy and low loss on the training set serve as strong indicators of its efficacy. 
Together with the real-world clinical data used for validation, this dataset's diversity and size highlight the model's resilience and ability to reliably identify and distinguish between a variety of neurological disorders.
